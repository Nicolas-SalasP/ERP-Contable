...
## Incidente real — archivos subidos (logos, adjuntos) invisibles para el navegador (detectado 2026-07-14)

**Síntoma:** el logo de empresa nunca se veía en el frontend ni en el PDF de cotización, sin importar cuántas veces se resubiera desde el admin. El upload en sí funcionaba (el archivo se guardaba correctamente en `$SHARED/storage/app/public/...`).

**Causa raíz:** `storage:link --force` corre correctamente y crea el symlink `$APP_LINK/public/storage → $SHARED/storage/app/public` (por eso el backend, que resuelve rutas vía `public_path()` de Laravel — o sea, relativo a `$APP_LINK`, no a `$ERP_PUBLIC` — sí encontraba el logo embebido en el PDF... salvo que el `Log::error` silencioso de `EmpresaService::actualizarLogo()` escondiera otro problema en producción, no confirmado). El problema real y confirmado es el navegador: pide el logo por URL directa (`https://dominio/storage/...`), que el servidor web resuelve contra `$ERP_PUBLIC` (`public_html`), **no** contra `$APP_LINK/public`. El deploy solo copiaba `index.php`, `favicon.ico` y `robots.txt` a `$ERP_PUBLIC` — nunca un symlink de `storage` — así que cualquier archivo servido por URL directa (logos, adjuntos, PDFs guardados) devolvía 404 en el docroot real, deploy tras deploy.

**Fix:** `ln -sfn "$SHARED/storage/app/public" "$ERP_PUBLIC/storage"` agregado en ambos jobs (staging y producción), justo después de copiar los 3 archivos estáticos. Es un symlink de una *subcarpeta* dentro de `public_html` (no del docroot completo), que no choca con la limitación ya documentada arriba sobre symlinkear `public_html` entero. También se sacó el `2>/dev/null` de `storage:link --force` — antes tragaba cualquier fallo real en silencio.

**Pendiente de verificar en el próximo deploy:** confirmar por SSH que `~/domains/.../public_html/storage` es ahora un symlink válido, y que la URL del logo carga en el navegador.
