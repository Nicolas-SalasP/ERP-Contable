        ...
        <div className="max-w-7xl mx-auto p-6 md:p-8">
                            ...
                            <tbody className="divide-y divide-slate-100">
                                {liquidaciones.length === 0 && (
                                    <tr><td colSpan={7} className="px-4 py-10 text-center text-slate-400">
                                        <i className="fas fa-receipt text-2xl mb-2 block" />
                                        Sin liquidaciones para {nombreMes(filtros.mes)} {filtros.anio}.
                                    </td></tr>
                                )}
                                {liquidaciones.map((liq) => (
                                    <tr key={liq.id} className="hover:bg-slate-50">
                                        ...
                                        <td className="px-4 py-3 text-slate-600">{nombreMes(liq.mes)} {liq.anio}</td>
                                        <td className="px-4 py-3 text-right text-slate-700">{formatPesos(liq.total_haberes_imponibles)}</td>
                                        <td className="px-4 py-3 text-right text-red-600">{formatPesos(liq.total_descuentos)}</td>
                                        <td className="px-4 py-3 text-right font-bold text-slate-900">{formatPesos(liq.liquido_a_pagar)}</td>
            ...
            <PanelModal abierto={modalCalc} titulo="Calcular liquidación" icono="fas fa-calculator" onClose={() => setModalCalc(false)}>
                <form onSubmit={calcular} className="grid sm:grid-cols-2 gap-3">
                    ...
                    <div>
                        <label className="block text-xs font-semibold text-slate-600 mb-1">Horas extra</label>
                    ...
                    <div>
                        <label className="block text-xs font-semibold text-slate-600 mb-1">Remuneraciones variables</label>
                    ...
                    <div className="sm:col-span-2">
                        <label className="block text-xs font-semibold text-slate-600 mb-1">APV voluntario</label>
                    ...
                    <div className="space-y-4">
                        <div className="flex items-center justify-between">
                            <div>
                                ...
                                <p className="text-sm text-slate-500">{nombreMes(detalle.mes)} {detalle.anio}</p>
                            </div>
                            ...
                            <table className="w-full text-sm">
                                <tbody className="divide-y divide-slate-100">
                                    {(detalle.detalles ?? []).map((d) => (
                                        <tr key={d.id}>
                                            <td className="px-4 py-2 text-slate-700">{d.nombre_concepto}</td>
                        ...
                        <div className="grid grid-cols-2 gap-2 text-sm">
                            ...
                            <div className="bg-slate-50 rounded-lg p-3">
                                <p className="text-xs text-slate-500">Total descuentos</p>
