        ...
        <div className="max-w-6xl mx-auto p-6 md:p-8">
            ...
            <div className="flex gap-1 mb-5 border-b border-slate-200">
                {TABS.map((t) => (
                    <button key={t.id} onClick={() => setTab(t.id)}
                        className={`px-4 py-2.5 text-sm font-semibold border-b-2 -mb-px transition-colors flex items-center gap-2 ${tab === t.id ? 'border-emerald-600 text-emerald-700' : 'border-transparent text-slate-500 hover:text-slate-700'}`}>
                    ...
                    !vigente ? (
                        ...
                        <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-6">
                            <div className="flex items-center justify-between mb-4">
                                <h3 className="font-bold text-slate-900">Vigente desde {formatFecha(vigente.vigente_desde)}</h3>
                            ...
                            {vigente.afp_comisiones_json && (
                                <div className="mt-4">
                                    <p className="text-xs font-semibold text-slate-600 mb-2">Comisiones AFP</p>
                                ...
                                <tbody className="divide-y divide-slate-100">
                                    ...
                                    {indicadores.map((ind) => (
                                        <tr key={ind.id} className="hover:bg-slate-50">
                                            <td className="px-4 py-3 text-slate-700">{nombreMes(ind.mes)} {ind.anio}</td>
                                            <td className="px-4 py-3 text-right text-slate-700">{formatPesos(ind.uf_valor)}</td>
                                            <td className="px-4 py-3 text-right text-slate-700">{formatPesos(ind.utm_valor)}</td>
                                            <td className="px-4 py-3 text-right text-slate-600">{formatPesos(ind.uta_valor)}</td>
                                        </tr>
                                    ))}
                                </tbody>
                            </table>
                                ...
                                <tbody className="divide-y divide-slate-100">
                                    ...
                                    {tabla.map((tr) => (
                                        <tr key={tr.id} className="hover:bg-slate-50">
                                            <td className="px-4 py-3 text-slate-700">{tr.orden}</td>
                                            <td className="px-4 py-3 text-right text-slate-600">{formatNumero(tr.desde_utm)}</td>
                                            <td className="px-4 py-3 text-right text-slate-600">{tr.hasta_utm ? formatNumero(tr.hasta_utm) : '∞'}</td>
                                            <td className="px-4 py-3 text-right font-medium text-slate-900">{formatNumero(tr.tasa * 100, 1)}%</td>
                                            <td className="px-4 py-3 text-right text-slate-600">{formatNumero(tr.factor_deduccion_utm)}</td>
                                        </tr>
                                    ))}
                                </tbody>
                            </table>
            ...
            <PanelModal abierto={modalInd} titulo="Registrar indicador mensual" icono="fas fa-chart-line" onClose={() => setModalInd(false)}>
                <form onSubmit={guardarIndicador} className="grid sm:grid-cols-2 gap-3">
                    ...
                    <div>
                        <label className="block text-xs font-semibold text-slate-600 mb-1">UTM *</label>
                    ...
                    <div className="sm:col-span-2 flex justify-end gap-2 pt-2 border-t border-slate-200">
                        <button type="button" onClick={() => setModalInd(false)} className="px-4 py-2 rounded-lg border border-slate-300 text-slate-600 font-semibold text-sm hover:bg-slate-50">Cancelar</button>
