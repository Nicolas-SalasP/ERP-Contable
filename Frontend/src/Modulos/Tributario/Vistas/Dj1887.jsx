...
const Dj1887 = () => {
    ...
    const [anio40Horas, setAnio40Horas]       = useState(anioActual);
    ...
    const cargarHistorial = useCallback(async () => {
        if (!puedeVer) return;
        try {
            const resp = await dj1887.listar();
    ...
    const handleGenerar = async () => {
        ...
        setCargando(true);
        try {
            const resp = await dj1887.generar(anio, anio40Horas);
            ...
            mostrarMensaje('exito', `DJ 1887 año ${anio} generada con ${nuevo.cantidad_registros ?? 0} registro(s).`);
        } catch (err) {
            const msg = err?.response?.data?.mensaje ?? err?.response?.data?.message ?? 'Error al generar la DJ 1887.';
    ...
    const handleDescargar = async (envioId, anioEnvio) => {
        ...
        try {
            const blob = await dj1887.descargar(envioId);
            const url = URL.createObjectURL(blob);
            const a = document.createElement('a');
            a.href = url;
            a.download = `DJ1887_${anioEnvio}.txt`;
            a.click();
            URL.revokeObjectURL(url);
            // Recargar para reflejar estado DESCARGADO
            await cargarHistorial();
        } catch (_) {
            // toast global
        } finally {
    ...
    return (
        <div className="max-w-5xl mx-auto p-6 md:p-8">
            ...
            <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700 p-5 mb-6">
                <h2 className="text-sm font-bold text-slate-700 dark:text-slate-300 mb-3 uppercase tracking-wide">Generar DJ 1887</h2>
                <div className="flex flex-wrap items-end gap-3">
                    <div>
                        <label htmlFor="dj1887-anio" className="block text-xs font-semibold text-slate-600 dark:text-slate-400 mb-1">Año tributario</label>
                        <select
                            id="dj1887-anio"
                            value={anio}
                    ...
                    <div>
                        <label htmlFor="dj1887-anio-40-horas" className="block text-xs font-semibold text-slate-600 dark:text-slate-400 mb-1">Año inicio 40h (Ley 21.561)</label>
                        <select
                            id="dj1887-anio-40-horas"
                            value={anio40Horas}
                            onChange={(e) => setAnio40Horas(Number(e.target.value))}
                            className={inputCls}
                        >
                            {ANIOS.map((a) => <option key={a} value={a}>{a}</option>)}
                        </select>
                    ...
                    {puedeProcesar && (
                        ...
                        >
                            {cargando
                                ...
                                : <><i className="fas fa-cogs" /> Generar DJ 1887</>
                            }
                        </button>
                    )}
                </div>
            ...
            <div className="bg-amber-50 border border-amber-200 rounded-xl px-4 py-3 mb-6 text-sm text-amber-800 flex gap-2">
                ...
                <span>
                    Las rentas se declaran en valores nominales (sin actualización IPC).
                    El archivo generado debe subirse manualmente al portal de Declaraciones Juradas del SII usando Clave Tributaria.
                </span>
            </div>
            ...
            {envioActivo ? (
                <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700 p-5 mb-6">
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-4 text-sm">
                        ...
                        <div className="bg-slate-50 dark:bg-slate-900 rounded-lg p-3">
                            <div className="text-xs text-slate-500 dark:text-slate-400 mb-1">Año 40h</div>
                            <div className="font-bold text-slate-900 dark:text-slate-100">{envioActivo.anio_40_horas ?? '—'}</div>
                        </div>
                        <div className="bg-slate-50 dark:bg-slate-900 rounded-lg p-3">
                        ...
                        <div className="pt-4 border-t border-slate-200 dark:border-slate-700">
                            ...
                            <div className="flex flex-wrap items-end gap-3">
                                <div className="flex-1 min-w-[200px]">
                                    <label htmlFor="dj1887-folio-confirmacion" className="block text-xs font-semibold text-slate-600 dark:text-slate-400 mb-1">
                                        Folio de presentación SII (opcional)
                                    </label>
                                    <input
                                        id="dj1887-folio-confirmacion"
                                        type="text"
                                        value={folioConfirmacion}
                                        onChange={(e) => setFolioConf(e.target.value)}
                                        maxLength={100}
                                        placeholder="Ej: 2024-DJ1887-000123"
                                        className={inputCls}
                                    />
                                </div>
                                <button
                ...
                <div className="bg-slate-50 border border-dashed border-slate-300 rounded-2xl py-10 text-center text-slate-400 mb-6">
                    ...
                    No hay declaraciones DJ 1887 generadas.
            ...
            <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700 overflow-hidden">
                ...
                {envios.length === 0 ? (
                    <div className="py-10 text-center text-slate-400 text-sm">
                        {historialCargado ? 'Sin declaraciones DJ 1887 aún.' : 'Cargando...'}
                    </div>
                ) : (
                        ...
                        <table className="w-full text-sm">
                            <thead className="sticky top-0 z-10 bg-slate-50 dark:bg-slate-900 text-slate-600 dark:text-slate-400 text-xs uppercase">
                                <tr>
                                    ...
                                    <th className="px-4 py-3 text-right font-semibold">Año 40h</th>
