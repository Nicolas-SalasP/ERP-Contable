...
const Dj1947 = () => {
    ...
    const cargarHistorial = useCallback(async () => {
        if (!puedeVer) return;
        try {
            const resp = await dj1947.listar();
    ...
    const handleGenerar = async () => {
        ...
        setCargando(true);
        try {
            const resp = await dj1947.generar(anio);
            ...
            mostrarMensaje('exito', `DJ 1947 año ${anio} generada con ${nuevo.cantidad_registros ?? 0} registro(s).`);
        } catch (err) {
            const msg = err?.response?.data?.mensaje ?? err?.response?.data?.message ?? 'Error al generar la DJ 1947.';
    ...
    const handleValidar = async () => {
        ...
        setValidando(true);
        try {
            const resp = await dj1947.validar(envioActivo.id);
            const data = resp?.data ?? resp;
            const actualizado = data?.envio ?? data;
            setEnvios((prev) => prev.map((e) => (e.id === envioActivo.id ? actualizado : e)));
            setSeleccionado(actualizado);
            mostrarMensaje('exito', 'DJ 1947 validada correctamente. Sin errores.');
        } catch (err) {
            const data = err?.response?.data;
            const msg = data?.mensaje ?? data?.message ?? 'Error al validar la DJ 1947.';
            mostrarMensaje('error', msg);
        } finally {
            setValidando(false);
        }
    ...
    const handleDescargar = async (envioId, anioEnvio) => {
        ...
        try {
            const blob = await dj1947.descargar(envioId);
            const url = URL.createObjectURL(blob);
            const a = document.createElement('a');
            a.href = url;
            a.download = `DJ1947_${anioEnvio}.txt`;
            a.click();
    ...
    const handleConfirmarPresentacion = async () => {
        ...
        setConfirmando(true);
        try {
            const resp = await dj1947.confirmarPresentacion(envioActivo.id, folioConfirmacion || null);
    ...
    return (
        <div className="max-w-5xl mx-auto p-6 md:p-8">
            ...
            <div className="bg-amber-50 border border-amber-200 rounded-xl px-4 py-3 mb-6 text-sm text-amber-800 flex gap-2">
                ...
                <span>
                    Esta DJ aplica únicamente a empresas en régimen Propyme Transparente (14D N°8).
                    Verifique el régimen tributario de su empresa antes de generar la declaración.
                    Los propietarios deben estar registrados en <strong>Empresa &rarr; Propietarios</strong>.
                </span>
            </div>
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-5 mb-6">
                <h2 className="text-sm font-bold text-slate-700 mb-3 uppercase tracking-wide">Generar DJ 1947</h2>
                    ...
                    {puedeProcesar && (
                        ...
                        >
                            {cargando
                                ...
                                : <><i className="fas fa-cogs" /> Generar DJ 1947</>
                            }
                        </button>
                    )}
                </div>
            ...
            {envioActivo ? (
                            ...
                            <div className="flex flex-wrap items-end gap-3">
                                <div className="flex-1 min-w-[200px]">
                                    ...
                                    <input
                                        ...
                                        maxLength={100}
                                        placeholder="Ej: 2024-DJ1947-000123"
                                        className={inputCls}
                                    />
                                </div>
                                <button
                ...
                <div className="bg-slate-50 border border-dashed border-slate-300 rounded-2xl py-10 text-center text-slate-400 mb-6">
                    ...
                    No hay declaraciones DJ 1947 generadas.
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
                ...
                {envios.length === 0 ? (
                    <div className="py-10 text-center text-slate-400 text-sm">
                        {historialCargado ? 'Sin declaraciones DJ 1947 aún.' : 'Cargando...'}
                    </div>
                ) : (
