...
const IvaWarningModal = ({ isOpen, onClose, onConfirm, calculado, ingresado }) => {
    ...
    return (
        <div className="fixed inset-0 bg-black bg-opacity-60 flex items-center justify-center z-50 p-4 animate-fade-in">
            <div className="bg-white dark:bg-slate-800 rounded-lg shadow-2xl p-6 max-w-md w-full border-t-4 border-yellow-500">
                ...
                <div className="bg-gray-50 dark:bg-slate-900 p-3 rounded mb-4 text-sm grid grid-cols-2 gap-4">
                    <div>
                        <span className="block text-gray-500 dark:text-slate-400 text-xs">Calculado (19%)</span>
                        <span className="font-bold text-gray-500 dark:text-slate-400 text-lg">${formatCurrency(calculado)}</span>
                    </div>
                    <div>
                        <span className="block text-gray-900 dark:text-slate-100 text-xs">Ingresado (Real)</span>
                ...
                <div className="flex justify-end space-x-3 mt-6">
                    <button
                        onClick={onClose}
                        className="px-4 py-2 text-gray-600 dark:text-slate-400 hover:bg-gray-100 dark:hover:bg-slate-700 rounded text-sm"
                    >
                        Cancelar
                    </button>
                    <button
