...
const RegistroFactura = () => {
    ...
    const handleNextStep = async () => {
        if (currentStep === 1) {
            if (!formData.proveedorId || !formData.numeroFactura || !formData.montoBruto) {
                alert("Por favor complete los campos obligatorios del Paso 1.");
                return;
            }
    ...
    return (
        <div className="max-w-5xl mx-auto bg-white rounded-lg shadow-xl overflow-hidden mt-6 border border-gray-200 font-sans">
            ...
            {/* CAMBIO 1: GenericModal -> ModalGenerico */}
            <ModalGenerico
            ...
            {/* CAMBIO 2: GenericModal -> ModalGenerico */}
            <ModalGenerico
                isOpen={!!successData}
                type="success"
            ...
            <div className="bg-gray-50 px-8 py-5 flex justify-between border-t border-gray-200 gap-4">
                ...
                {currentStep < 3 ? (
                    <button
                        ...
                        className="px-8 py-2.5 bg-slate-900 text-white rounded-lg font-bold shadow-lg hover:bg-slate-800 disabled:opacity-50 flex items-center justify-center gap-2"
                    >
                        {checkingDuplicate ? (
                            <>
                    ...
                    <button onClick={handlePreSave} disabled={saving || ivaInvalido} className="px-8 py-2.5 bg-emerald-600 text-white rounded-lg font-bold shadow-lg hover:bg-emerald-700 disabled:opacity-50 flex items-center justify-center gap-2">
                        {saving ? (
                            <>
                                <svg className="animate-spin h-5 w-5 text-white" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                                    <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle>
