                    ...
                    {proveedoresFiltrados.map(prov => (
                        <div key={prov.id} onClick={() => seleccionarProveedor(prov.id)} className="flex items-center justify-between p-4 hover:bg-blue-50 dark:hover:bg-slate-700 cursor-pointer border-b border-slate-50 dark:border-slate-700 transition-colors">
                            <div className="flex items-center gap-4">
                                ...
                                <div>
                                    <p className="font-bold text-slate-800 dark:text-slate-200 text-sm">{prov.razon_social}</p>
                                    <p className="text-[10px] text-slate-500 dark:text-slate-400 font-mono">RUT: {prov.rut || 'N/A'} <span className="mx-2">•</span> COD: {prov.codigo_interno}</p>
                                </div>
                            </div>
    ...
    const modalAnticipoJSX = modalAnticipoAbierto && (
        <div className="fixed inset-0 bg-slate-900/80 z-[100] flex items-center justify-center p-4 animate-fade-in" onClick={() => setModalAnticipoAbierto(false)}>
            <div className="bg-white dark:bg-slate-800 w-full max-w-lg rounded-xl shadow-xl overflow-hidden border border-slate-300 dark:border-slate-700 flex flex-col" onClick={e => e.stopPropagation()}>
                ...
                <div className="p-6 space-y-5 bg-slate-50 dark:bg-slate-900">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-5">
                        <div>
                            ...
                            <input type="date" value={formAnticipo.fecha} onChange={e => setFormAnticipo({ ...formAnticipo, fecha: e.target.value })} className="w-full bg-white dark:bg-slate-700 border border-slate-300 dark:border-slate-600 p-2.5 rounded font-medium text-slate-800 dark:text-slate-200 outline-none focus:border-blue-500 transition-colors" />
                        </div>
                        <div>
                            <label className="block text-xs font-bold text-slate-600 dark:text-slate-400 uppercase mb-1">Monto Solicitado ($)</label>
                            <input type="number" placeholder="Ej: 500000" value={formAnticipo.monto} onChange={e => setFormAnticipo({ ...formAnticipo, monto: e.target.value })} className="w-full bg-white dark:bg-slate-700 border border-slate-300 dark:border-slate-600 p-2.5 rounded font-mono font-bold text-slate-800 dark:text-slate-200 outline-none focus:border-blue-500 transition-colors" />
                        </div>
                    </div>
                    <div>
                        <label className="block text-xs font-bold text-slate-600 dark:text-slate-400 uppercase mb-1">Motivo / Referencia Operativa</label>
                        <input type="text" placeholder="Ej: Pago adelantado OC-8821" value={formAnticipo.referencia} onChange={e => setFormAnticipo({ ...formAnticipo, referencia: e.target.value })} className="w-full bg-white dark:bg-slate-700 border border-slate-300 dark:border-slate-600 p-2.5 rounded font-medium text-slate-800 dark:text-slate-200 outline-none focus:border-blue-500 transition-colors" />
                    </div>
                </div>

                <div className="bg-white dark:bg-slate-800 border-t border-slate-200 dark:border-slate-700 p-4 shrink-0 flex justify-end gap-3">
                    <button onClick={() => setModalAnticipoAbierto(false)} className="px-5 py-2 text-slate-600 dark:text-slate-400 border border-slate-300 dark:border-slate-600 font-bold hover:bg-slate-100 dark:hover:bg-slate-700 rounded transition-colors text-sm">Cancelar</button>
        ...
        return (
            <div className="max-w-7xl mx-auto p-4 md:p-8 font-sans h-full flex flex-col">
                ...
                <div className="flex-1 flex flex-col items-center justify-center text-center mt-20">
                    ...
                    <h1 className="text-3xl font-bold text-slate-800 dark:text-slate-200 mb-2">Visor del Proveedor</h1>
        ...
        <div className="fixed inset-0 bg-slate-900/80 backdrop-blur-sm z-[100] flex items-center justify-center p-4 animate-fade-in" onClick={() => setModalCruceAbierto(false)}>
            <div className="bg-white dark:bg-slate-800 w-full max-w-5xl rounded-xl shadow-xl overflow-hidden flex flex-col max-h-[95vh] border border-slate-300 dark:border-slate-700 animate-slide-down" onClick={e => e.stopPropagation()}>
                ...
                <div className="flex flex-col lg:flex-row flex-1 overflow-hidden">
                    ...
                    <div className="flex-1 flex flex-col border-r border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800">
                        <div className="bg-slate-50 dark:bg-slate-900 px-5 py-3 border-b border-slate-200 dark:border-slate-700 flex justify-between items-center shrink-0">
                            <h3 className="font-bold text-slate-800 dark:text-slate-200 text-sm flex items-center gap-2 uppercase tracking-wide">
                                <i className="fas fa-file-invoice text-rose-500"></i> Deudas Vigentes
                            </h3>
                            <span className="bg-slate-200 dark:bg-slate-700 text-slate-700 dark:text-slate-300 text-[10px] font-bold px-2 py-0.5 rounded shadow-sm">{facturasDeuda.length} docs</span>
                        </div>
                        <div className="flex-1 overflow-y-auto p-4 custom-scrollbar bg-white dark:bg-slate-800">
                    ...
                    <div className="flex-1 flex flex-col bg-white dark:bg-slate-800">
                        <div className="bg-slate-50 dark:bg-slate-900 px-5 py-3 border-b border-slate-200 dark:border-slate-700 flex justify-between items-center shrink-0">
                            ...
                            <span className="bg-slate-200 dark:bg-slate-700 text-slate-700 dark:text-slate-300 text-[10px] font-bold px-2 py-0.5 rounded shadow-sm">{ncVigentes.length + anticiposVigentes.length} docs</span>
                        </div>
                        <div className="flex-1 overflow-y-auto p-4 custom-scrollbar bg-white dark:bg-slate-800">
                                    ...
                                    {ncVigentes.map(nc => {
                                        ...
                                        return (
                                            ...
                                            >
                                                ...
                                                <div className="flex-1">
                                                    <p className="font-bold text-slate-800 dark:text-slate-200 text-sm leading-none flex items-center gap-2">
                                    ...
                                    {anticiposVigentes.map(ant => {
                                        ...
                                        return (
                                            ...
                                            >
                                                ...
                                                <p className="font-bold text-slate-800 dark:text-slate-200 text-base">{formatCurrency(saldoMostrar)}</p>
                                            </div>
                                        );
                                    })}
                                </div>
                ...
                <div className="bg-slate-50 dark:bg-slate-900 border-t border-slate-200 dark:border-slate-700 p-4 shrink-0 flex flex-col md:flex-row justify-between items-center gap-4">
                    <div className="flex items-center gap-4 w-full md:w-auto">
                        <div>
                            <p className="text-[10px] font-bold text-slate-500 dark:text-slate-400 uppercase mb-0.5">Total Deudas</p>
                            <p className="text-lg font-bold text-slate-800 dark:text-slate-200">{formatCurrency(totalSelCargos)}</p>
                        </div>
                        <div className="text-slate-300 text-xl font-light">-</div>
                        <div>
                            <p className="text-[10px] font-bold text-slate-500 dark:text-slate-400 uppercase mb-0.5">Total a Favor</p>
                            <p className="text-lg font-bold text-slate-800 dark:text-slate-200">{formatCurrency(totalSelAbonos)}</p>
                        </div>
                        <div className="text-slate-300 text-xl font-light">=</div>
                        <div className="bg-white dark:bg-slate-800 px-3 py-1 rounded border border-slate-200 dark:border-slate-700">
                            <p className="text-[10px] font-bold text-slate-500 dark:text-slate-400 uppercase mb-0.5">Diferencia</p>
                            <p className={`text-lg font-bold ${difCruce === 0 ? 'text-slate-500' : 'text-slate-800 dark:text-slate-200'}`}>
        ...
        <div className="max-w-7xl mx-auto p-4 md:p-6 font-sans text-slate-800 dark:text-slate-200 animate-fade-in pb-20">
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4 mb-8">
                <div>
                    ...
                    <h1 className="text-3xl font-black text-slate-900 dark:text-slate-100 tracking-tight">Ficha 360° del Proveedor</h1>
                </div>

                <div className="flex flex-wrap gap-2">
                    <button onClick={abrirModalCruce} className="bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-600 text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700 font-bold py-2 px-4 rounded text-sm transition-colors flex items-center gap-2">
                        <i className="fas fa-random"></i> Cruzar Documentos
                    </button>
                    <button onClick={() => setModalAnticipoAbierto(true)} className="bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-600 text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700 font-bold py-2 px-4 rounded text-sm transition-colors flex items-center gap-2">
                ...
                <div className="bg-white dark:bg-slate-800 p-4 border-b border-slate-100 dark:border-slate-700 flex flex-wrap gap-4 items-center">
                    <div className="flex-1 min-w-full sm:min-w-[200px]">
                        <div className="relative w-full">
                            ...
                            <input type="text" placeholder="Filtrar N° Doc o Referencia..." value={filtroNumero} onChange={(e) => setFiltroNumero(e.target.value)} className="w-full pl-9 pr-3 py-2 border border-slate-300 dark:border-slate-600 rounded text-sm outline-none focus:border-blue-500 bg-white dark:bg-slate-700 text-slate-700 dark:text-slate-300" />
                        </div>
                    </div>
