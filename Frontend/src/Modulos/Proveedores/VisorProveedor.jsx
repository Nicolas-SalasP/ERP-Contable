...
const VisorProveedor = () => {
    ...
    const [tabActiva, setTabActiva] = useState('facturas'); // 'facturas' o 'anticipos'
    
    // --- ESTADOS: Buscador Spotlight ---
    const [modalAbierto, setModalAbierto] = useState(false);
    const [listaProveedores, setListaProveedores] = useState([]);
    const [terminoBusqueda, setTerminoBusqueda] = useState('');
    const inputBusquedaRef = useRef(null);

    // --- ESTADOS: Modal Solicitar Anticipo ---
    ...
    // --- ESTADOS: Modal Cruce de Anticipos ---
    const [modalCruceAbierto, setModalCruceAbierto] = useState(false);
    const [facturaSeleccionada, setFacturaSeleccionada] = useState(null);
    const [formCruce, setFormCruce] = useState({ anticipo_id: '', monto: '' });
    ...
    // Atajos de Teclado (Ctrl+2 y Escape)
    useEffect(() => {
    ...
    const cargarListaCompleta = async () => {
        ...
        } catch (error) { console.error("Error al cargar proveedores"); }
    };
    ...
    const guardarAnticipo = async () => {
        ...
        try {
            ...
            const payload = { ...formAnticipo, proveedor_id: id };
            const res = await api.post('/proveedores/anticipos', payload);
            
            if (res.success) {
                Swal.fire('¡Solicitud Creada!', 'El anticipo está listo para ser pagado desde la cartola o nómina.', 'success');
                setModalAnticipoAbierto(false);
    ...
    // --- ACCIONES: CRUCE DE ANTICIPOS ---
    const abrirModalCruce = (factura) => {
        setFacturaSeleccionada(factura);
        setFormCruce({ anticipo_id: '', monto: factura.monto_bruto }); // Sugerimos cruzar el total de la factura
        setModalCruceAbierto(true);
    };

    const ejecutarCruce = async () => {
        if (!formCruce.anticipo_id || !formCruce.monto) {
            return Swal.fire('Faltan Datos', 'Debe seleccionar un anticipo y digitar un monto válido.', 'warning');
        }
        try {
            Swal.fire({ title: 'Cruzando contabilidad...', didOpen: () => Swal.showLoading() });
            const res = await api.post(`/facturas/${facturaSeleccionada.id}/cruzar-anticipo`, formCruce);
            
            if (res.success) {
                Swal.fire('¡Pagada!', 'La factura fue cruzada con el anticipo exitosamente.', 'success');
                setModalCruceAbierto(false);
                cargarFicha(id);
            }
        } catch (error) {
            Swal.fire('Error', error.response?.data?.mensaje || 'Error al cruzar el saldo.', 'error');
        }
    };


    // --- FILTROS Y RENDERS MODALES ---
    ...
    const modalCruceJSX = modalCruceAbierto && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-[100] flex items-center justify-center animate-fade-in p-4" onClick={() => setModalCruceAbierto(false)}>
            <div className="bg-white w-full max-w-md rounded-3xl shadow-2xl overflow-hidden animate-slide-down" onClick={e => e.stopPropagation()}>
                <div className="bg-slate-900 px-6 py-4 flex justify-between items-center text-white">
                    <h3 className="font-black text-lg flex items-center gap-2"><i className="fas fa-random text-indigo-400"></i> Cruzar Saldo a Favor</h3>
                    <button onClick={() => setModalCruceAbierto(false)} className="text-slate-400 hover:text-white"><i className="fas fa-times text-xl"></i></button>
                </div>
                <div className="p-6 space-y-4">
                    <div className="bg-slate-50 p-4 rounded-xl border border-slate-200 text-center">
                        <p className="text-xs font-bold text-slate-500 uppercase">Factura a Pagar</p>
                        <p className="font-black text-2xl text-slate-800">#{facturaSeleccionada?.numero_factura}</p>
                        <p className="text-sm font-mono text-slate-600 mt-1">Total: {formatCurrency(facturaSeleccionada?.monto_bruto)}</p>
                    </div>
                    <div>
                        <label className="block text-xs font-bold text-slate-500 uppercase mb-1">Seleccionar Anticipo</label>
                        <select value={formCruce.anticipo_id} onChange={e => setFormCruce({...formCruce, anticipo_id: e.target.value})} className="w-full bg-white border border-slate-300 p-3 rounded-xl font-bold text-slate-700 outline-none focus:border-indigo-500">
                            <option value="">Elegir un anticipo vigente...</option>
                            {datos?.anticipos.filter(a => a.estado === 'VIGENTE').map(ant => (
                                <option key={ant.id} value={ant.id}>
                                    Ref: {ant.referencia} | Disponible: ${parseFloat(ant.saldo_disponible).toLocaleString('es-CL')}
                                </option>
                            ))}
                        </select>
                    </div>
                    <div>
                        <label className="block text-xs font-bold text-slate-500 uppercase mb-1">Monto a Descontar ($)</label>
                        <input type="number" max={facturaSeleccionada?.monto_bruto} value={formCruce.monto} onChange={e => setFormCruce({...formCruce, monto: e.target.value})} className="w-full bg-white border border-slate-300 p-3 rounded-xl font-mono font-black text-slate-800 outline-none focus:border-indigo-500" />
                        <p className="text-[10px] text-slate-400 mt-1">* Se sugiere el total de la factura o el saldo restante.</p>
                    </div>
                </div>
                <div className="bg-slate-50 p-4 border-t border-slate-100 flex justify-end gap-3">
                    <button onClick={() => setModalCruceAbierto(false)} className="px-6 py-2.5 text-slate-500 font-bold hover:bg-slate-200 rounded-xl transition-colors">Cancelar</button>
                    <button onClick={ejecutarCruce} className="bg-slate-900 hover:bg-black text-white font-black px-8 py-2.5 rounded-xl shadow-lg transition-transform hover:-translate-y-0.5">Aplicar Cruce</button>
                </div>
            </div>
        </div>
    );

    // --- RENDERIZADO VISTA VACÍA (SIN ID) ---
    if (loading && !datos) {
        return (
    ...
    return (
        <div className="max-w-7xl mx-auto p-4 md:p-6 font-sans text-slate-800 animate-fade-in pb-20">
            ...
            {/* TARJETA MAESTRA DEL PROVEEDOR (LA OSCURA) */}
            ...
            {/* SISTEMA DE PESTAÑAS (FACTURAS VS ANTICIPOS) */}
                            ...
                            <table className="w-full text-left text-sm whitespace-nowrap">
                                <thead className="bg-white border-b border-slate-100">
                                    <tr>
                                        ...
                                        <th className="px-8 py-4 font-black text-slate-400 text-[10px] uppercase tracking-widest">N° Factura</th>
                                        <th className="px-8 py-4 font-black text-slate-400 text-[10px] uppercase tracking-widest text-right">Monto Neto</th>
                                        <th className="px-8 py-4 font-black text-slate-400 text-[10px] uppercase tracking-widest text-right">Total Bruto</th>
                                        <th className="px-8 py-4 font-black text-slate-400 text-[10px] uppercase tracking-widest text-center">Estado</th>
                                        <th className="px-8 py-4 font-black text-slate-400 text-[10px] uppercase tracking-widest text-center">PDF</th>
                                        <th className="px-8 py-4 font-black text-slate-400 text-[10px] uppercase tracking-widest text-center">Acciones</th>
                                    </tr>
                                </thead>
                                <tbody className="divide-y divide-slate-50 bg-slate-50/20">
                                    {facturas.map((fac) => (
                                        <tr key={fac.id} className="hover:bg-indigo-50/40 transition-colors group">
                                            <td className="px-8 py-4 font-bold text-slate-500">{new Date(fac.fecha_emision).toLocaleDateString('es-CL')}</td>
                                            <td className="px-8 py-4 font-black text-slate-800 text-base"><span className="text-slate-300 font-normal mr-1">#</span>{fac.numero_factura}</td>
                                            <td className="px-8 py-4 text-right font-mono font-medium text-slate-500">{formatCurrency(fac.monto_neto)}</td>
                                            <td className="px-8 py-4 text-right font-mono font-black text-slate-900 text-base">{formatCurrency(fac.monto_bruto)}</td>
                                            <td className="px-8 py-4 text-center">
                                                {fac.estado === 'PAGADA' ? <span className="bg-emerald-100 text-emerald-700 text-[10px] font-black px-3 py-1.5 rounded-md uppercase tracking-widest border border-emerald-200 shadow-sm">Pagada</span> : fac.estado === 'ANULADA' ? <span className="bg-rose-100 text-rose-700 text-[10px] font-black px-3 py-1.5 rounded-md uppercase tracking-widest border border-rose-200 shadow-sm">Anulada</span> : <span className="bg-amber-100 text-amber-700 text-[10px] font-black px-3 py-1.5 rounded-md uppercase tracking-widest border border-amber-200 shadow-sm">Pendiente</span>}
                                            </td>
                                            <td className="px-8 py-4 text-center">
                                                {fac.archivo_pdf ? (
                                                    <a href={`${import.meta.env.VITE_API_URL.replace('/api', '')}/${fac.archivo_pdf}`} target="_blank" rel="noopener noreferrer" className="w-8 h-8 rounded-lg flex items-center justify-center bg-rose-50 border border-rose-200 text-rose-600 hover:bg-rose-500 hover:text-white transition-all shadow-sm mx-auto" title="Ver PDF" onClick={(e) => e.stopPropagation()}><i className="fas fa-file-pdf"></i></a>
                                                ) : (
                                                    <label className="w-8 h-8 rounded-lg flex items-center justify-center bg-slate-50 border border-slate-200 text-slate-400 hover:bg-indigo-50 hover:text-indigo-600 hover:border-indigo-200 transition-all shadow-sm cursor-pointer mx-auto" title="Adjuntar PDF" onClick={(e) => e.stopPropagation()}>
                                            ...
                                            <td className="px-8 py-4 text-center">
                                                {/* BOTÓN MÁGICO DE CRUCE (Solo aparece si la factura debe plata y hay anticipos vigentes) */}
                                                {fac.estado !== 'PAGADA' && anticiposVigentes > 0 && (
                                                    <button onClick={() => abrirModalCruce(fac)} className="bg-slate-900 hover:bg-black text-white text-xs font-bold px-4 py-2 rounded-lg shadow-md transition-transform hover:-translate-y-0.5">
                                                        <i className="fas fa-random mr-2"></i> Cruzar Saldo
                                                    </button>
                                                )}
                                                {fac.estado === 'PAGADA' && <span className="text-slate-300 text-xs italic"><i className="fas fa-check-double"></i> Cerrada</span>}
                                            </td>
                                        </tr>
                                    ))}
                                </tbody>
                                ...
                                <tbody className="divide-y divide-slate-50 bg-slate-50/20">
                                    {anticipos.map((ant) => (
                                        <tr key={ant.id} className="hover:bg-emerald-50/30 transition-colors group">
                                            ...
                                            <td className="px-8 py-4 font-medium text-slate-700">{ant.referencia || 'Sin referencia'}</td>
