...
class SoftDeletesTest extends TestCase
{
    ...
    protected function setUp(): void
    {
        ...
        $this->empresa = Empresa::create([
            'rut' => '77.123.456-7',
            'razon_social' => 'SoftDelete SpA',
            'regimen_tributario' => '14_D3',
        ]);
    }

    public function test_cliente_se_borra_de_forma_logica_y_es_recuperable(): void
    {
        $cliente = Cliente::create([
            'empresa_id' => $this->empresa->id,
            'rut' => '1.1.1.1-1',
            'razon_social' => 'Cliente Borrable',
            'estado' => 'ACTIVO',
        ]);
    ...
    public function test_proveedor_se_borra_de_forma_logica_y_es_recuperable(): void
    {
        $proveedor = Proveedor::create([
            'empresa_id' => $this->empresa->id,
            'razon_social' => 'Prov Borrable',
            'rut' => '2.2.2.2-2',
            'codigo_interno' => 'P-DEL',
            'pais_iso' => 'CL',
            'moneda_defecto' => 'CLP',
        ]);
    ...
    public function test_factura_se_borra_de_forma_logica_y_es_recuperable(): void
    {
        $proveedor = Proveedor::create([
            'empresa_id' => $this->empresa->id,
            'razon_social' => 'Prov F',
    ...
    public function test_activo_fijo_se_borra_de_forma_logica_y_es_recuperable(): void
    {
    ...
    public function test_asiento_contable_se_borra_de_forma_logica_y_es_recuperable(): void
    {
        $asiento = AsientoContable::create([
            'empresa_id' => $this->empresa->id,
            'fecha' => now(),
            'glosa' => 'Asiento Borrable',
            'numero_comprobante' => 'AS-DEL',
    ...
    public function test_cotizacion_se_borra_de_forma_logica_y_es_recuperable(): void
    {
        ...
        $cotizacion = Cotizacion::create([
            ...
            'nombre_cliente' => 'Cliente Cot',
            'estado_id' => 1,
            'numero_cotizacion' => 'COT-DEL',
            'subtotal' => 100,
            'monto_neto' => 100,
            'monto_iva' => 19,
            'monto_total' => 119,
        ...
        $eliminada = Cotizacion::withTrashed()->find($id);

        $this->assertNull(Cotizacion::find($id));
        $this->assertNotNull($eliminada);
        $this->assertTrue($eliminada->trashed());
        $this->assertSame($this->empresa->id, $eliminada->empresa_id);
        $this->assertSoftDeleted('cotizaciones', ['id' => $id]);

        $eliminada->restore();

        $restaurada = Cotizacion::find($id);

        $this->assertNotNull($restaurada);
        $this->assertFalse($restaurada->trashed());
        $this->assertSame($this->empresa->id, $restaurada->empresa_id);
    }
}
