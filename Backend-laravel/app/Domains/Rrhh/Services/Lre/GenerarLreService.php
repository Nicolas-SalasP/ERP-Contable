...
class GenerarLreService
{
    ...
    public function generar(int $empresaId, int $anio, int $mes): LreEnvio
    {
        ...
        $lineas = $liquidaciones->map(fn (Liquidacion $liq) => $this->construirLinea($liq, $empresa))->all();
    ...
    private function construirLinea(Liquidacion $liq, Empresa $empresa): LreLineaData
    {
        ...
        return new LreLineaData(
            ...
            codigoAfc: $codigoAfc,
            // Fuente unica es empresa->mutualidad (afiliacion es por empresa, no por trabajador).
            // Fallback al campo legacy por empleado solo si la empresa aun no fue migrada.
            codigoMutual: $empresa->mutualidad?->codigo_lre !== null
                ? (int) $empresa->mutualidad->codigo_lre
                : ($empleado->organismo_mutual_codigo ? (int) $empleado->organismo_mutual_codigo : null),
            sueldo: $sueldo,
