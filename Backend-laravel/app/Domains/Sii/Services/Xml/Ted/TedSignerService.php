...
/** Firma el bloque DD del TED con RSA-SHA1 usando la clave privada (RSASK) del CAF; la firma resultante se inserta en <FRMT algoritmo="SHA1withRSA">. El SII verifica esta firma contra RSAPUBK incluida en el CAF, validando que el emisor del DTE es el legitimo titular del CAF autorizado. NO usa XMLDSig — el TED del SII Chile usa una firma RSA cruda con SHA1 codificada en base64, implementada con openssl_sign nativo (sin xmlseclibs). */
class TedSignerService
{
    ...
    /**
     * Firma el string DD con RSA-SHA1 y retorna la firma en base64; IMPORTANTE: el byte-exact string que entra aqui es el byte-exact que el SII va a verificar, si difiere en UN byte del DD que aparece en el XML final del DTE, el SII rechaza el DTE.
     *
     * @param string $ddXml bloque <DD>...</DD> serializado en bytes finales (encoding ISO-8859-1 segun spec SII).
     *
    ...
    public function firmarDd(string $ddXml, SiiCaf $caf): string
    {
        ...
        // En PHP 8+, $clavePrivada es OpenSSLAsymmetricKey y se libera por GC; openssl_pkey_free esta deprecated y emite warning.
        unset($clavePrivada);
    ...
    /** Verifica una firma RSA-SHA1 contra la clave publica del CAF; utilidad para tests y validacion local, el SII hace su propia verificacion. */
