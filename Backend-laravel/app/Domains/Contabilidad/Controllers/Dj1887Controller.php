...
use App\Domains\Contabilidad\Services\Dj\Dj1887Service;
use App\Http\Controllers\Controller;
use Illuminate\Http\JsonResponse;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\Storage;

class Dj1887Controller extends Controller
{
    public function __construct(
        private readonly DjEngine      $engine,
        private readonly Dj1887Service $dj1887,
    ) {}
    ...
    public function index(Request $request): JsonResponse
    {
        ...
        $envios = DjEnvio::where('empresa_id', $empresaId)
            ->porCodigo('1887')
            ->orderByDesc('anio')
            ->get(['id', 'anio', 'estado', 'cantidad_registros', 'anio_40_horas', 'folio_presentacion', 'presentado_at', 'created_at']);
    ...
    public function generar(Request $request): JsonResponse
    {
        $data = $request->validate([
            ...
            'anio_40_horas' => 'nullable|integer|min:2024|max:2099',
        ]);
        ...
        try {
            ...
            return response()->json([
                'mensaje' => 'DJ 1887 generada correctamente.',
                'data'    => $envio,
            ], 201);
        } catch (DjException $e) {
    ...
    public function descargar(Request $request, DjEnvio $djEnvio)
    {
        ...
        $nombreArchivo = "DJ1887_{$djEnvio->anio}.txt";
    ...
    public function confirmarPresentacion(Request $request, DjEnvio $djEnvio): JsonResponse
    {
        ...
        return response()->json([
            'mensaje' => 'DJ 1887 marcada como presentada.',
            'data'    => $djEnvio->fresh(),
        ]);
    }
}
