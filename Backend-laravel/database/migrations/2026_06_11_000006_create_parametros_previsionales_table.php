...
return new class extends Migration {
    public function up(): void
    {
        ...
        Schema::create('parametros_previsionales', function (Blueprint $table) {
            ...
            // Las tasas se guardan como porcentaje (10 = 10%), por eso decimal(7,4):
            // decimal(5,4) sólo admite hasta 9.9999 y MySQL estricto rechaza el default 10.
            $table->decimal('afp_cotizacion_pct', 7, 4)->default(10.0000);  // 10%
            ...
            $table->decimal('afp_sis_pct', 7, 4)->default(1.6200);  // 1.62% (desde abril 2026)
            ...
            // Salud (Ley 18.469)
            $table->decimal('salud_fonasa_pct', 7, 4)->default(7.0000);  // 7%

            // AFC — Seguro de Cesantía (Ley 19.728)
            $table->decimal('afc_indefinido_trabajador_pct', 7, 4)->default(0.6000);   // 0.6%
            $table->decimal('afc_indefinido_empleador_pct', 7, 4)->default(2.4000);    // 2.4%
            $table->decimal('afc_plazo_fijo_trabajador_pct', 7, 4)->default(0.0000);   // 0%
            $table->decimal('afc_plazo_fijo_empleador_pct', 7, 4)->default(3.0000);    // 3%
            ...
            $table->decimal('gratificacion_tope_mensual_factor', 7, 4)->default(4.7500);
            ...
            $table->decimal('cotizacion_adicional_empleador_pct', 7, 4)->default(1.0000);

            // Mutual (accidentes del trabajo) — Ley 16.744
            // El porcentaje varía por actividad económica; se configura por empresa si difiere
            $table->decimal('mutual_cotizacion_basica_pct', 7, 4)->default(0.9000);   // 0.9% básico
